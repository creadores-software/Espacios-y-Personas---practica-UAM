...
export default {
  ...
  methods: {
    eliminar($slug) {
      this.$axios.delete("/api/person/" + $slug,this.config).then((res) => {});
    },
    cleanErrorsForm() {
    ...
    submit() {
      ...
      if (this.labelSubmit == "Guardar") {
        ...
        if (!this.hasErrors) {
          ...
          this.$axios
            .get("/api/person/buildSlug/" + this.full_name,this.config)
            .then((res) => {
              this.personSlug = res.data;
              this.$axios
                .get("/api/person/slug/" + this.personSlug,this.config)
                .then((res) => {
                ...
                .catch((err) => {
                  ...
                  this.$axios
                    .post("/api/person", this.form,this.config)
                    .then((res) => {
                      this.$axios
                        .get("/api/person/id/" + this.personSlug,this.config)
                        .then((res) => {
        ...
        if (!this.hasErrors) {
          ...
          this.$axios
            ...
            .then((res) => {
              ...
              this.form.slug = this.personSlug;
              this.$axios
                .put("/api/person/" + this.firstPersonSlug, this.form,this.config)
                .then((res) => {
                  oruga.notification.open({
                    duration: 5000,
